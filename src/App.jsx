        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={()=>{if(!emp.name){showToast('اختر عضواً أولاً','warning');return;}setShowEv(true);}}
              className={cx('flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition-all hover:opacity-80',T.b.purple)}>
            ...
            <button onClick={()=>{if(!isActive){showToast('العضو غير نشط — لا يستحق خدمات الإعانة','error');return;}if(!emp.name){showToast('اختر عضواً أولاً','warning');return;}jumpToAid(emp);}}
              className={cx('flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition-all hover:opacity-80',T.b.sky)}>
    ...
    <div className="space-y-5" style={{animation:'fadeIn .35s both'}}>
      <div className="flex justify-between items-center">
        ...
        <button onClick={()=>setShowAdd(!showAdd)}
          className="flex items-center gap-2 px-4 py-2.5 bg-teal-500 hover:bg-teal-400 text-slate-900 font-bold rounded-xl text-sm transition-all shadow-md">
          ...
          <table className="w-full text-sm text-right">
            ...
            <tbody>
              {boardMembers.length===0?(
                <tr><td colSpan="4" className={cx('p-8 text-center',T.muted)}>لا يوجد أعضاء مسجلين</td></tr>
              ):boardMembers.map(m=>(
                <tr key={m.id} className={cx('border-b transition-colors',T.div,T.row)}>
                  ...
                  <td className="p-4 text-center">
                    <button onClick={()=>setDm(m)} className={cx('inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border transition-all hover:opacity-80',T.b.amber)}>
          ...
          <div className={cx('rounded-2xl p-6 max-w-2xl w-full shadow-2xl border',T.card)} style={{animation:'fadeIn .3s both'}}>
            ...
            <div className="mt-4 flex justify-end">
              <button onClick={()=>{printVoucher({vType:'كشف مستحقات عضو مجلس إدارة',vNum:`BD-${dm.id}`,date:new Date().toLocaleDateString('ar-EG'),party:dm.name,amount:'2500',notes:`بدل حضور + بدل انتقال — ${dm.role}`,checkNum:null});}}
                className={cx('flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-xl border transition-all hover:opacity-80',T.b.teal)}>
  ...
  return (
    <div className="space-y-5" style={{animation:'fadeIn .35s both'}}>
      <div className="flex justify-between items-start">
        ...
        <button onClick={reset} className={cx('flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition-all hover:opacity-80',T.btn)}><RefreshCw size={12}/> نموذج جديد</button>
      </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {wf!=='draft'&&(
            <div className="flex gap-2 flex-wrap">
              <button onClick={doPrint} className={cx('flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border transition-all hover:opacity-80',T.b.teal)}>
              ...
              {isAid&&selEmp&&(
                <button onClick={()=>printAidRequest({emp:selEmp,aidCat,aidRel,incDate:incD,amount:amt,date:txD,notes})}
                  className={cx('flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border transition-all hover:opacity-80',T.b.sky)}>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {!locked&&(
          <button onClick={()=>showToast('تم حفظ المسودة','info')}
            className={cx('flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold border transition-all hover:opacity-80',T.btn)}>
            <FileText size={13}/> حفظ مسودة
          </button>
        )}
        <button onClick={doPrint}
          className={cx('flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold border transition-all hover:opacity-80',T.b.teal)}>
          <Printer size={13}/> معاينة وطباعة
        </button>
        {isAid&&selEmp&&(
          <button onClick={()=>printAidRequest({emp:selEmp,aidCat,aidRel,incDate:incD,amount:amt,date:txD,notes})}
            className={cx('flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold border transition-all hover:opacity-80',T.b.sky)}>
    ...
    <div className="space-y-5" style={{animation:'fadeIn .35s both'}}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className={cx('p-5 rounded-2xl border space-y-4',T.card)}>
          ...
          <button onClick={add} className="w-full flex items-center justify-center gap-2 bg-teal-500 hover:bg-teal-400 text-slate-900 font-bold py-2.5 rounded-xl text-sm transition-all shadow-md">
        ...
        <div className={cx('md:col-span-2 rounded-2xl border overflow-hidden',T.card)}>
          ...
          {expenses.length>0&&(
            <div className={cx('p-4 border-t flex flex-col sm:flex-row gap-3',T.div)}>
              <button onClick={()=>showToast('تم إغلاق العهدة وترحيل المتبقي للخزينة','success')}
                className="flex-1 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl text-sm transition-all shadow-md">
                <CheckCircle2 size={14}/> إغلاق العهدة واعتماد التسوية
              </button>
              <button onClick={doPrint} className={cx('flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-bold border transition-all hover:opacity-80',T.b.teal)}>
...
/* ══════════════════════════════════════════════════════════════
   MAIN APP CONTENT (Wraps actual UI to ensure hooks render correctly)
══════════════════════════════════════════════════════════════ */
const AppContent = ({ user, setUser }) => {
  ...
  // 👉 FIXED: goTab is now defined safely at the top-level 
  ...
  // Conditional Rendering (Safe from Hook violations)
  if (!user) {
  ...
  return (
    <>
      ...
      <header className={cx('fixed top-0 w-full h-14 border-b z-40 flex items-center justify-between px-4 md:px-6',T.hdr,T.div)}>
        <div className="flex items-center gap-3">
          <button className={cx('lg:hidden p-1.5 rounded-lg border transition-all hover:bg-slate-500/10',T.btn)} onClick={()=>setNavOpen(o=>!o)} aria-label="القائمة">
        ...
        <div className="flex items-center gap-2">
          {/* Theme toggle */}
          <button onClick={toggle} title={dark?'وضع نهاري':'وضع ليلي'}
            className={cx('flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-xs font-bold transition-all hover:opacity-80',T.btn)}>
            {dark?<Sun size={13}/>:<Moon size={13}/>}
            <span className="hidden sm:inline">{dark?'نهاري':'ليلي'}</span>
          </button>
          {/* Notification */}
          <button onClick={()=>showToast('لا توجد تنبيهات جديدة','info')}
            className={cx('relative p-2 rounded-lg border transition-all hover:opacity-80',T.btn)}>
            <Bell size={15}/>
            <span className="absolute top-1 right-1 w-1.5 h-1.5 bg-rose-500 rounded-full"/>
          </button>
          {/* User */}
          <div className={cx('flex items-center gap-2 px-3 py-1.5 rounded-lg border cursor-default',T.btn)}>
            ...
            <span className={cx('text-sm font-semibold hidden md:block truncate max-w-[120px]',T.text)}>{user.name}</span>
          </div>
      ...
      {/* ── Mobile overlay ── */}
      {navOpen&&<div className="fixed inset-0 bg-black/50 z-30 lg:hidden backdrop-blur-sm" onClick={()=>setNavOpen(false)}/>}

      <div className="flex mt-14 min-h-[calc(100vh-56px)]" dir="rtl">
        {/* ── SIDEBAR ── */}
        <nav className={cx(
          'fixed top-14 bottom-0 right-0 w-52 z-30 flex flex-col p-3 gap-0.5 border-l transition-transform duration-300 overflow-y-auto shadow-2xl lg:shadow-none',
          T.nav, T.div,
            ...
            return (
              <button key={id} onClick={()=>goTab(id)}
                className={cx('flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all text-right',
                  active?(dark?'bg-teal-500/10 text-teal-400 border border-teal-500/20 shadow-md shadow-teal-500/5':'bg-teal-50 text-teal-700 border border-teal-200'):
                  cx(T.btn,'border border-transparent hover:bg-slate-500/10'))}>
          ...
          <div className={cx('mt-auto pt-3 border-t',T.div)}>
            <div className={cx('px-3 py-2 rounded-xl text-xs font-semibold border cursor-default flex items-center justify-center gap-2',T.btn)}>
              {user.role==='treasurer' ? <><Shield size={12} className="text-amber-400"/> أمين الصندوق</> : <><UserCircle size={12} className="text-sky-400"/> مدخل بيانات</>}
            </div>
          </div>
        </nav>
...
/* ══════════════════════════════════════════════════════════════
   ROOT APP (Theme Provider Wrapper)
══════════════════════════════════════════════════════════════ */
export default function App() {
  ...
  const CSS = `
    ...
    *, *::before, *::after { font-family: 'Cairo', sans-serif; box-sizing: border-box; }
    ...
    ::-webkit-scrollbar-thumb { background: ${dark ? '#334155' : '#cbd5e1'}; border-radius: 99px; }
    ::-webkit-scrollbar-thumb:hover { background: #0d9488; }
    select option { background: ${dark ? '#1e293b' : '#ffffff'}; color: ${dark ? '#f1f5f9' : '#0f172a'}; }
    body { background-color: ${dark ? '#0c1220' : '#f1f5f9'}; }
  `;
